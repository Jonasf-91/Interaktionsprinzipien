...
class FourConnectBoard (private val calc : FourConnectCalculator, var field : Array<Array<Int>>, var currentPlayer : Int = 2, private val computerId : Int = 2, private val humanId : Int = 1 ){
    ...
    private fun scorePosition(_row : Int, _column : Int, delta_x : Int, delta_y : Int) : Int{
        ...
        var humanPoints = 0
        val winningListComputer = mutableListOf<Pair<Int,Int>>()
        val winningListHuman = mutableListOf<Pair<Int,Int>>()
    ...
    fun score() : Int{
        ...
        for(row in 0 until calc.rows-3){
            for(column in 0 until calc.columns){
                val score = scorePosition(row, column, 0, 1)
        ...
        for(row in 0 until calc.rows){
            for(column in 0 until  calc.columns-3){
                val score = scorePosition(row, column, 1, 0)
                if(score == calc.scoreEnd) return calc.scoreEnd
                if(score == -calc.scoreEnd) return -calc.scoreEnd
        ...
        for(row in 0 until calc.rows-3){
            for(column in 0 until  calc.columns-3){
                val score = scorePosition(row, column, 1, 1)
                if(score == calc.scoreEnd) return calc.scoreEnd
                if(score == -calc.scoreEnd) return -calc.scoreEnd
